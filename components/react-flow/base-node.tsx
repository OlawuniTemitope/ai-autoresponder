...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    <div
      className={cn(
        "bg-card text-card-foreground relative  border-muted-foreground rounded-sm border hover:bg-accent",
        className,
      )}
      tabIndex={0}
      {...props}
    ...
    >
      ...
      {
        status === "success" && (
          <CheckCircle2Icon className="absolute right-0.5 bottom-0.5 size-2
